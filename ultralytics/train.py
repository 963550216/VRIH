...
if __name__ == "__main__":
    ...
    # model = YOLO('ultralytics/cfg/models/v8/yolov8-c2f-UniRe.yaml')
    # model = YOLO('ultralytics/cfg/models/v8/yolov8-dysample-SCAM.yaml')
    model = YOLO("ultralytics/cfg/models/v8/yolov8-dysample-SCAM-c2f-UniRe.yaml")
    model.load("./yolov8n.pt")  # 注释则不加载
    results = model.train(
        data="./my_data2.yaml",  # 数据集配置文件的路径
        epochs=100,  # 训练轮次总数
