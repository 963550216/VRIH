...
class MultiScaleContextBlock(nn.Module):
    def __init__(self, in_channels, inter_channels=None):
        ...
        # ---------------------------
        # Step 2: 定义三个并行处理分支 (用于 Concat)
        # 为了让最后的 Add 操作成立，Concat 后的总通道数必须等于 in_channels。
        # 因此，我们将 in_channels 平均分配给这三个分支。
        # ---------------------------
        c1 = in_channels // 3
        c2 = in_channels // 3
        c3 = in_channels - c1 - c2  # 剩下的通道给最后一个分支，处理除不尽的情况
        ...
        # 膨胀卷积通常使用 3x3 核。Padding = dilation 以保持空间尺寸不变。
    ...
    def forward(self, x):
        ...
        # 1. 顶部 1x1 卷积
        feat = self.conv_head(x)  # [B, inter_channels, H, W]

        # 2. 三个处理分支
        b_out = self.branch_conv1x1(feat)
        c_out = self.branch_dilated_2(feat)
        d_out = self.branch_dilated_4(feat)

        # 3. Concat
        # 拼接后 shape: [B, in_channels, H, W]
        concat_out = torch.cat([b_out, c_out, d_out], dim=1)

        # 4. 注意力分支 (左侧路径)
        # Global Avg Pooling: [B, inter_channels, 1, 1]
        gap_out = self.gap(feat)
        # Attn weights: [B, in_channels, 1, 1]
        attn_map = self.conv_attn(gap_out)

        # 5. 乘法 (Multiplication)
        # 利用广播机制: [B, C, H, W] * [B, C, 1, 1]
        modulated = concat_out * attn_map

        # 6. 加法 (Add) - 残差连接
        output = modulated + residual
