...
# Plot bar charts
bars1 = plt.bar(x - width / 2, train_counts, width, label="Training Set", color="skyblue", edgecolor="navy", alpha=0.8)
bars2 = plt.bar(
    x + width / 2, val_counts, width, label="Validation Set", color="lightcoral", edgecolor="darkred", alpha=0.8
)
...
plt.title("Class Distribution Comparison in SAR-Aircraft-1.0 Dataset", fontsize=16, fontweight="bold", pad=20)
...
print("\nImbalance Analysis:")
print(
    f"Training Set - Max: {max_train}({categories[train_counts.index(max_train)]}) | "
    f"Min: {min_train}({categories[train_counts.index(min_train)]}) | "
    f"Imbalance Ratio: {max_train / min_train:.2f}:1"
)

print(
    f"Validation Set - Max: {max_val}({categories[val_counts.index(max_val)]}) | "
